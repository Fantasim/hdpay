...
// Standard BIP-39 test mnemonic (12-word — known BIP-84 test vector).
...
func TestKeyService_DeriveBTCPrivateKey_KnownVector(t *testing.T) {
	...
	// the resulting public key produces the same address as hd.DeriveBTCAddress.
	...
	want, err := hd.DeriveBTCAddress(masterKey, 0, net)
	if err != nil {
		t.Fatal(err)
	}
...
func TestKeyService_DeriveBTCPrivateKey_MatchesPublicDerivation(t *testing.T) {
	...
	// matches the address generated by hd.DeriveBTCAddress.
	path := writeTempMnemonic(t, testMnemonic24)
	ks := NewKeyService(path, "mainnet")
	...
	for _, idx := range []uint32{0, 1, 5, 42} {
		t.Run("index_"+itoa(int(idx)), func(t *testing.T) {
			...
			expectedAddr, err := hd.DeriveBTCAddress(masterKey, idx, net)
			if err != nil {
				t.Fatal(err)
			}
...
func TestKeyService_DeriveSOLPrivateKey_KnownVector(t *testing.T) {
	...
	// Verify against hd.DeriveSOLAddress.
	seed, err := hd.MnemonicToSeed(testMnemonic24)
	if err != nil {
		t.Fatal(err)
	}
	expectedAddr, err := hd.DeriveSOLAddress(seed, 0)
	if err != nil {
		t.Fatal(err)
	}
