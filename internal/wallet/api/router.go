...
import (
	...
	"github.com/Fantasim/hdpay/internal/wallet/api/middleware"
...
func NewRouter(database *db.DB, cfg *config.Config, sc *scanner.Scanner, hub *scanner.SSEHub, ps *price.PriceService, sendDeps *handlers.SendDeps, staticFS fs.FS) chi.Router {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Get("/health/providers", handlers.GetProviderHealth(database, sc.GetAllProviderMetrics))
		...
		r.Route("/send", func(r chi.Router) {
			...
			r.Get("/sweep/{sweepID}", handlers.GetSweepStatus(sendDeps))
	...
	if staticFS != nil {
		...
		r.NotFound(httputil.SPAHandler(staticFS))
	}
